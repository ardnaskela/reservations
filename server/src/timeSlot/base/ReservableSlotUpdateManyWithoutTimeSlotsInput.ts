/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ReservableSlotWhereUniqueInput } from "../../reservableSlot/base/ReservableSlotWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class ReservableSlotUpdateManyWithoutTimeSlotsInput {
  @Field(() => [ReservableSlotWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ReservableSlotWhereUniqueInput],
  })
  connect?: Array<ReservableSlotWhereUniqueInput>;

  @Field(() => [ReservableSlotWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ReservableSlotWhereUniqueInput],
  })
  disconnect?: Array<ReservableSlotWhereUniqueInput>;

  @Field(() => [ReservableSlotWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ReservableSlotWhereUniqueInput],
  })
  set?: Array<ReservableSlotWhereUniqueInput>;
}
export { ReservableSlotUpdateManyWithoutTimeSlotsInput };
